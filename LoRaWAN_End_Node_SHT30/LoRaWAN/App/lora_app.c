...
#define APP_PERIODIC_TX_INTERVAL_MS   60000
/* USER CODE END PD */
...
static Adafruit_SHT31 g_sht31;                 // SHT31 ????
volatile float g_sht31_temp_c = 0;           // ??????(�C)
volatile float g_sht31_hum_rh = 0;           // ??????(%RH)
...
void LoRaWAN_Init(void)
{
  ...
  if (EventType == TX_ON_TIMER)
  {
    ...
    UTIL_TIMER_Create(&TxTimer, APP_PERIODIC_TX_INTERVAL_MS, UTIL_TIMER_ONESHOT, OnTxTimerEvent, NULL);
...
static void SendTxData(void)
{
  ...
  LmHandlerErrorStatus_t status = LORAMAC_HANDLER_ERROR; // 用于记录是坦戝功坑�??
  uint8_t batteryLevel = GetBatteryLevel();              // VDD
  float temperature = 0;                                 // temp
	float humidity = 0;																		 // hum
  UTIL_TIMER_Time_t nextTxIn = 0;

#ifdef CAYENNE_LPP
  uint8_t channel = 0;
#else
  uint8_t battery = 100; // 默认电池电針�?100
  uint32_t i = 0;
#endif /* CAYENNE_LPP */

  if (!Adafruit_SHT31_begin(&g_sht31, &hi2c2, 0x44)) {
    APP_LOG(TS_ON, VLEVEL_L, "SHT31 init failed\r\n");
  } else {
    APP_LOG(TS_ON, VLEVEL_L, "SHT31 init ok\r\n");
  }

  Adafruit_SHT31_readTemperature(&g_sht31);
  Adafruit_SHT31_readHumidity(&g_sht31);
  temperature = g_sht31.temp;
  humidity = g_sht31.humidity;
	APP_LOG(TS_ON, VLEVEL_L, "VDD is %d\r\n", batteryLevel);
	APP_LOG(TS_ON, VLEVEL_L, "temperature is %d\r\n",g_sht31.temp);
	APP_LOG(TS_ON, VLEVEL_L, "humidity is %d\r\n", g_sht31.humidity);
  ...
  if ((LmHandlerParams.ActiveRegion != LORAMAC_REGION_US915) && (LmHandlerParams.ActiveRegion != LORAMAC_REGION_AU915) && (LmHandlerParams.ActiveRegion != LORAMAC_REGION_AS923))
  {
  ...
  /* �? float 温度转杢�? centi-degrees 的整数，靿兝�? float 使用佝移擝作导致编译错误 */
  int16_t temp_centi = (int16_t)(temperature * 100.0f); /* e.g. 23.45 -> 2345 */
  uint16_t hum_centi = (uint16_t)(humidity * 100u);     /* 若有真实湿度值请先赋�? humidity */

  /* 4) 填充 AppData.Buffer �? 网关 parseMultiSensor 覝求的顺庝：
